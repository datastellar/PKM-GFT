...
Dalam konteks Indonesia, berbagai inisiatif seperti Satu Data Indonesia, pengembangan portal data nasional, serta platform interoperabilitas SPBE telah menunjukkan kemajuan dalam aspek standar metadata dan integrasi teknis. Namun mekanisme tata kelola data lintas instansi yang operasionalâ€”termasuk penetapan kepemilikan data, tanggung jawab kualitas data, dan forum koordinasi pengelolaan dataâ€”belum berkembang secara memadai. BRIN (2023) mengungkap bahwa pergerakan parsial ini berujung pada resistensi birokrasi, di mana mekanisme tata kelola data lintas instansi yang menghubungkan semuanya secara operasional masih sangat terbatas.
...
Gagasan ini menawarkan pendekatan berbeda dengan menempatkan tata kelola data sebagai fondasi utama sebelum integrasi teknis dilakukan. Model yang diusulkan mengintegrasikan prinsip federated data governance, domain-oriented data architecture (data mesh), serta kerangka interoperabilitas SPBE dalam satu arsitektur sistem pemerintahan yang terkoordinasi. Model arsitektur 4 layer ini menjamin sistem milik berbagai instansi tetap otonom namun terkoneksi penuh secara standar.
...
Akibatnya, bahkan inisiatif Satu Data Indonesia (SDI) yang diamanatkan Perpres No. 39/2019 pun mengalami stagnasi. BRIN (2023) mengungkap bahwa program SDI menghadapi hambatan serius: perbedaan standar metadata lintas kementerian membuat integrasi sulit secara komputasi, sementara pendekatan implementasi yang *top-down* tanpa forum kolaborasi teknis antar pemilik data justru menimbulkan resistensi. Dari pola ini teridentifikasi satu akar masalah yang konsisten: mekanisme tata kelola data lintas instansi belum tersedia â€” tidak ada kejelasan tentang pemilik data, tidak ada standar metadata yang mengikat, dan tidak ada forum yang memiliki otoritas untuk menyepakati standar tersebut.
...
Berbeda dengan pendekatan yang selama ini berfokus pada integrasi teknis terlebih dahulu, model ini secara eksplisit menempatkan tata kelola data sebagai fondasi utama. Dengan demikian, interoperabilitas sistem tidak dibangun di atas integrasi aplikasi semata, tetapi di atas kesepakatan institusional mengenai kepemilikan data, standar metadata, serta mekanisme koordinasi lintas instansi. Klischewski dan Scholl (2008) menegaskan bahwa interoperabilitas teknis saja tidak akan menghasilkan manfaat nyata jika kualitas dan standar data dari sistem-sistem sumber masih berbeda-beda. Oleh karena itu, model ini dirancang secara berjenjang dari fondasi ke atas, sebagaimana digambarkan dalam diagram berikut:
...
Salah satu pemborosan paling nyata dari fragmentasi SPBE adalah duplikasi pembangunan sistem dengan fungsi serupa di kementerian yang berbeda. Laporan Ombudsman membeberkan fakta bahwa pemerintah saat ini menanggung beban operasional atas lebih dari **27.400 aplikasi** yang mayoritas beroperasi secara *silo*. Di sisi lain, riset global menunjukkan proyek IT pemerintah rata-rata berpotensi mengalami pembengkakan biaya (*cost overrun*) hingga 18% bila manajemen datanya buruk (Heeks, 2003).

Apabila model *Federated Data Governance* ini dieksekusi, komponen layanan dasar akan beralih menjadi skema *shared services* — satu standar API, satu direktori data nasional, tanpa instansi memelihara *server* redundan. Menggunakan asumsi konservatif: jika model ini berhasil mendepresiasi 30% saja dari total 27.400 aplikasi yang redundan tersebut, pemerintah dapat menghapus beban pemeliharaan untuk lebih dari **8.200 aplikasi duplikat**. Dengan asumsi moderat biaya operasional dan pemeliharaan server Rp 50 juta per aplikasi per tahun, potensi efisiensi APBN yang diselamatkan dapat mencapai **lebih dari Rp 410 Miliar setiap tahunnya** — anggaran yang sebelumnya terbakar untuk birokrasi *server*, dapat direalokasikan langsung menjadi *output* layanan kepada masyarakat.
...
Pada puncaknya, model ini menempatkan Indonesia sebagai negara yang tak lagi mengejar ketertinggalan teknologi, melainkan memiliki infrastruktur data yang tangguh dan terintegrasi penuh. Pada akhirnya, keberhasilan transformasi digital pemerintahan tidak semata ditentukan oleh kecanggihan teknologi yang digunakan, melainkan oleh kemampuan institusi dalam mengelola dan berbagi data secara terkoordinasi. Dengan menempatkan tata kelola data sebagai fondasi utama, model interoperabilitas ini berupaya memastikan bahwa digitalisasi pemerintahan berjalan secara berkelanjutan, aman, dan memberikan manfaat nyata bagi masyarakat demi menyongsong visi Indonesia Emas 2045.
